...
function hocwp_theme_dev_end_of_working_hours() {
	...
	if ( $morning || $noon || $afternoon || $evening ) {
		...
		$msg .= '<script>setInterval(function(){window.location.href = window.location.href;},3e4);</script>';
	...
	if ( ! is_admin() ) {
		...
		if ( false !== ( $minute = get_transient( $tr_name ) ) ) {
			if ( ! wp_doing_ajax() && ! wp_doing_cron() ) {
				...
				$message = sprintf( 'You should take a break and relax for %d minutes.', $minute );
